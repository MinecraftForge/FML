...
/**
 * Class transformer for Minecraft Froge's {now FML's} EventBus,
 * this takes care of some of the tedious boiler plate code 
 * that needs to be added to every event to ensure it manages 
 * it's lists properly and dispatches events. 
 * 
 * @author LexManos
 */
public class EventTransformer implements IClassTransformer
{
    private static final String BASE_EVENT = "net.minecraftforge.event.Event";
    private static final String LISTENER_LIST = "net.minecraftforge.event.ListenerList";
    public EventTransformer(){}

    @Override
    public byte[] transform(String name, String transformedName, byte[] bytes)
    {
        if (bytes == null || name.equals(BASE_EVENT) || name.startsWith("net.minecraft.") || name.indexOf('.') == -1)
        {
            return bytes;
        }
    ...
    {
        Class<?> parent = getClass().getClassLoader().loadClass(classNode.superName.replace('/', '.'));
        Class<?> event = getClass().getClassLoader().loadClass(BASE_EVENT);
        if (!event.isAssignableFrom(parent))
        {
            return false;
        }
        ...
        Class<?> listenerListClazz = Class.forName(LISTENER_LIST, false, getClass().getClassLoader());
